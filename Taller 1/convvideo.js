var img;
const convMatrix = [[0, -1, 0], [-1, 5, -1], [0, -1, 0]];
let convImg;
let fps;
...
function setup() {
  fps = createGraphics(100, 100);

  createCanvas(img.width * 2, img.height * 3);
...
function draw() {
  ...
  fps.background(255);
  fps.textSize(32);
  fps.text(`fps: ${frameRate().toFixed(0)}`, 20, 60);
  image(fps, 1280, 60);
...
function updatedPixels(i, orig) {
  return (
    pixelConvoluted(i - 4 - orig.width * 4, 0, 0, orig) +
